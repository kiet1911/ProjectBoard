...
export default function CreateContainer({
  ...
  const status = useCreateContainer(useShallow((state) => state.status));
  const node = useCreateContainer(useShallow((state) => state.node));
  const close = useCreateContainer(useShallow((close) => close.close));
  return (
    <>
      {status && (
        <>
