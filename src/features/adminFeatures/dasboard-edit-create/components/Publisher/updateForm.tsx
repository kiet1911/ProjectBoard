...
export default function UpdateForm({
  ...
  const closeForm = useUpdateContainer(useShallow((state) => state.close));
  const notification = useToastNotification(useShallow((state) => state.add));
  const mutation = useMutation({
    mutationKey: ["data_update_publisher",form],
    mutationFn: async()=>{
        const res = await publisher_service_dashboard.updatePublisher(form);
        return res;
    },
    onSuccess: (config)=>{
  ...
  const handleConfirm= useCallback(async()=>{
          const confirmValue = await confirm("Are you sure you want to update this Publisher");
          //  console.log(form);
          if(confirmValue){
              // console.log("success");
