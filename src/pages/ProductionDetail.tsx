...
export default function ProductionDetail() {
  ...
  const [images, setImages] = useState<
    { id: string; alt: string; img_Url: string; is_Thumbnail: boolean }[]
  >([]);
  ...
  useEffect(() => {
    if (!detailBoardGame?.images) return;

    const arr = [...detailBoardGame.images];

    arr.sort((a, b) => Number(b.is_Thumbnail) - Number(a.is_Thumbnail));
            ...
            >
              ...
              <div className="  w-full p-0 gap-0 border-2 border-mist-800/20 rounded-tl-2xl rounded-tr-xl flex flex-row max-md:flex-col">
                <div className=" flex-1 rounded-tl-xl max-md:rounded-t-xl shrink-0 p-2 bg-(--footer-bg-color) flex flex-col justify-center max-md:items-center">
                  <img
                    src={
                      images[0]?.img_Url || "/Suspense/404NotFoundImage.png"
                    }
                    alt={images[0]?.alt || "No Image"}
                    className="object-contain h-50 p-1 border-2 rounded-lg border-mist-200/50"
                  />

                  <div className="w-full container flex gap-2 py-2 overflow-x-auto">
                    {images.length > 1 &&
                      images
                        .slice(1)
                        .map((img, index) => (
                          <img
                            key={img.id}
                            src={img.img_Url || "/Suspense/404NotFoundImage.png"}
                            alt={img.alt}
                            className="h-15 w-20 object-contain cursor-pointer border-2 rounded-lg border-mist-200/50"
                ...
                <div className=" flex-3 shrink-0 min-w-0 p-2 flex flex-col md:rounded-tr-xl gap-2 bg-white">
                  ...
                  <div className="flex flex-row items-center gap-2 font-bold mt-auto ">
                    ...
                    {detailBoardGame?.discount &&
                      detailBoardGame?.discount > 0 && (
                        <div className="flex-1 navbar-link  text-center">
                          {" "}
